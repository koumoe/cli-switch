...
async fn create_managed_openai_channel(
    db_path: std::path::PathBuf,
    name: &str,
    base_url: String,
    account_id: &str,
    ...
    storage::create_channel(
        ...
        storage::CreateChannel {
            ...
            managed_remote_account_id: Some(account_id.to_string()),
            managed_remote_resource_id: None,
            managed_remote_resource_name: None,
            managed_remote_group_name: None,
            managed_remote_group_id: None,
...
#[tokio::test]
async fn managed_openai_ticket_replaces_client_turn_state() {
    ...
    let account = storage::upsert_openai_account_tokens(
        db_path.clone(),
        Some("OpenAI ticket test".to_string()),
        storage::OpenAiAccountTokens {
            access_token: "oauth-access-token".to_string(),
            refresh_token: Some("oauth-refresh-token".to_string()),
            id_token: Some("id-token".to_string()),
            token_expires_at_ms: Some(i64::MAX),
            account_id: "chatgpt-account-ticket".to_string(),
            email: Some("ticket@example.com".to_string()),
            display_name: None,
            plan_type: Some("plus".to_string()),
        },
    )
    ...
    storage::update_app_settings(
        ...
        storage::AppSettingsPatch {
            ...
            openai_codex_ticket_fail_closed: Some(true),
            openai_codex_ticket_models: Some(vec!["gpt-5.6-sol".to_string()]),
            ..Default::default()
        },
    )
    .await
    ...
    assert_eq!(
        captured.as_slice(),
        [(ticket, String::new(), "chatgpt-account-ticket".to_string())]
    );
}

#[tokio::test]
async fn managed_openai_ticket_fail_closed_avoids_upstream_request() {
    let (base_url, calls) = spawn_upstream_counted(StatusCode::OK, r#"{"ok":true}"#).await;
    let db_path = temp_db_path();
    storage::init_db(&db_path).unwrap();
    let account = storage::upsert_openai_account_tokens(
        db_path.clone(),
        Some("OpenAI fail closed test".to_string()),
        storage::OpenAiAccountTokens {
            access_token: "oauth-access-token".to_string(),
            refresh_token: Some("oauth-refresh-token".to_string()),
            id_token: None,
            token_expires_at_ms: Some(i64::MAX),
            account_id: "chatgpt-account-fail-closed".to_string(),
            email: None,
            display_name: None,
            plan_type: None,
        },
    ...
    .await
    .expect_err("missing ticket must fail closed");
    assert!(matches!(
        error,
        proxy::ProxyError::CodexTicketUnavailable(model) if model == "gpt-5.6-sol"
    ));
    assert_eq!(calls.load(Ordering::Relaxed), 0);
}

#[tokio::test]
async fn managed_openai_ticket_fail_closed_fails_over_to_next_channel() {
    ...
    let account = storage::upsert_openai_account_tokens(
        db_path.clone(),
        Some("OpenAI failover ticket test".to_string()),
        storage::OpenAiAccountTokens {
            access_token: "oauth-access-token".to_string(),
            refresh_token: Some("oauth-refresh-token".to_string()),
            id_token: None,
            token_expires_at_ms: Some(i64::MAX),
            account_id: "chatgpt-account-failover-ticket".to_string(),
            email: None,
            display_name: None,
            plan_type: None,
        },
...
#[tokio::test]
async fn rejected_managed_openai_ticket_is_invalidated() {
    let (base_url, _) =
        spawn_upstream_counted(StatusCode::FORBIDDEN, r#"{"error":"bad ticket"}"#).await;
    let db_path = temp_db_path();
    storage::init_db(&db_path).unwrap();
    let account = storage::upsert_openai_account_tokens(
        db_path.clone(),
        Some("OpenAI ticket invalidation test".to_string()),
        storage::OpenAiAccountTokens {
            access_token: "oauth-access-token".to_string(),
            refresh_token: Some("oauth-refresh-token".to_string()),
            id_token: None,
            token_expires_at_ms: Some(i64::MAX),
            account_id: "chatgpt-account-ticket-invalidation".to_string(),
            email: None,
            display_name: None,
            plan_type: None,
        },
    ...
    let stored = storage::get_openai_codex_ticket(db_path, account.id, "gpt-5.6-sol".to_string())
        .await
        .unwrap()
        .unwrap();
