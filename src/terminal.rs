...
fn write_tool_shim(
    ...
    #[cfg(windows)]
    {
        ...
        let mut prefix = if dedup.is_empty() {
            String::new()
        } else {
    ...
    #[cfg(not(windows))]
    {
        ...
        if !dirs.is_empty() {
            ...
            expr.push_str(":\"$PATH\"");
            export_lines.push(expr);
        }
        for (key, value) in env_vars {
            export_lines.push(format!("export {key}={}", sh_single_quote(value)));
        }
...
/// Extra environment the generated shim exports before handing off to the tool.
///
/// Codex CLI shares `~/.codex` with the ChatGPT desktop app, so when isolation is
/// enabled we pin the CLI to its own directory here. The desktop app launches
/// from the GUI without the shell environment, so it keeps using `~/.codex`.
fn shim_env_vars(tool_bin: &str) -> anyhow::Result<Vec<(String, String)>> {
    if tool_bin != "codex" {
        return Ok(Vec::new());
    }
    let Some(dir) = crate::codex_home::env_override()? else {
        return Ok(Vec::new());
    };
    let value = dir
        .to_str()
        .ok_or_else(|| {
            anyhow::anyhow!("codex home path contains invalid utf-8: {}", dir.display())
        })?
        .to_string();
    Ok(vec![("CODEX_HOME".to_string(), value)])
}
...
mod tests {
    ...
    /// The shim is how `CODEX_HOME` reaches a CLI started from the user's own
    /// terminal, so the export has to land before the exec line.
    #[cfg(not(windows))]
    #[test]
    fn write_tool_shim_exports_env_vars_before_exec() {
        let path = tmp_file("codex-shim");
        write_tool_shim(
            &path,
            ...
            &[(
                ...
                "/home/u/.cliswitch/codex".to_string(),
            )],
        )
        .expect("write shim");
        ...
        let export = lines
            .iter()
            .position(|line| *line == "export CODEX_HOME='/home/u/.cliswitch/codex'")
            .expect("missing CODEX_HOME export");
        let exec = lines
            .iter()
            .position(|line| line.starts_with("exec "))
            .expect("missing exec line");
        assert!(export < exec, "export must precede exec: {content}");
        ...
        .expect("rewrite shim");
        let content = std::fs::read_to_string(&path).expect("read shim");
        assert!(!content.contains("CODEX_HOME"), "stale export: {content}");
