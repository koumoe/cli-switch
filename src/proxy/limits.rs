...
// repeat large tool manifests in a single physical line, so keep the parser cap well above the
// response log preview cap while remaining bounded.
pub(super) const MAX_SSE_BUF_BYTES: usize = 8 * 1024 * 1024;
...
pub(super) const MAX_OPENAI_RESPONSES_BOOTSTRAP_BYTES: usize = 2 * 1024 * 1024;
