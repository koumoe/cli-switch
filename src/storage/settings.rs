...
const KEY_DESKTOP_PET_ENABLED: &str = "desktop_pet_enabled";
...
pub struct AppSettingsPatch {
    ...
    pub desktop_pet_enabled: Option<bool>,
...
pub async fn get_app_settings(db_path: PathBuf) -> anyhow::Result<AppSettings> {
    with_conn(db_path, move |conn| {
        ...
        if let Some(v) = get_setting(conn, KEY_CHANNEL_RETRY_ENABLED)? {
            out.channel_retry_enabled = parse_bool_setting(
                KEY_CHANNEL_RETRY_ENABLED,
                &v,
                &mut has_invalid_values,
                out.channel_retry_enabled,
            );
        }
        ...
        if let Some(v) = get_setting(conn, KEY_DESKTOP_PET_ENABLED)? {
            out.desktop_pet_enabled = parse_bool_setting(
                KEY_DESKTOP_PET_ENABLED,
                &v,
                &mut has_invalid_values,
                out.desktop_pet_enabled,
            );
        }
...
pub async fn update_app_settings(
    ...
    with_conn(db_path2, move |conn| {
        ...
        if let Some(v) = patch.channel_retry_enabled {
            set_setting(
                conn,
                KEY_CHANNEL_RETRY_ENABLED,
                if v { "true" } else { "false" },
        ...
        if let Some(v) = patch.desktop_pet_enabled {
            set_setting(
                conn,
                KEY_DESKTOP_PET_ENABLED,
                if v { "true" } else { "false" },
